...
Workaround: `programs.nix-ld.enable = true` is set per-container in each
`.devenv/extra.nix` (and in the default `extra.nix` template generated by `devenv.sh`, so
new containers get it automatically). nix-ld provides a stub ELF interpreter so the
downloaded binaries run regardless of path.
...
If so, the `programs.nix-ld.enable` line in each container's `extra.nix` (and the template)
— and possibly `services.vscode-server` and its flake input — can be removed.
